...
class PriceComparatorTest {
    ...
    private List<Float> priceToFloatArray(List<Car> carList) {
        List<Float> carPriceInfo = new ArrayList();;
        for (Car car : carList){
            carPriceInfo.add(car.getPrice());
        }
        return carPriceInfo;
    }
    ...
    public void checkPriceComparator() {
        ...
        Collections.sort(carList,new PriceComparator()); //Sorting based on Price Comparator
        List<Float> actual = priceToFloatArray(carList);
